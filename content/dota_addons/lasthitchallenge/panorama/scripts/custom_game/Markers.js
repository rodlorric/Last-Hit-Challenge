 ...
 * Date: July 2015
 */
// original: https://github.com/Perryvw/PanoramaUtils

//Constant parameters
var MARKER_DURATION = 0.7;
var MARKER_UPDATE_INTERVAL = 1;
var MARKER_OFFSET = new Vector( -50, -100 );
...
var Marker = function( entIndex, name, percent ) {
    ...
    if (percent != "missed"){        
        if (percent > 0.75){
            ...
            this.label.style.animationName = "wobble";
            text = $.Localize( "#overlay_heavy_breathing" );
            this.label.style.animationDuration = "0.5s";
	...
	//this.label.style.transition = "transform " + MARKER_UPDATE_INTERVAL + "s linear 0.0ms";
...
Marker.prototype.PointOnScreen = function( target ) {
	...
	//this.label.style.transform = "translate3d(" + ( target.x + 40 ) + "px, " + ( target.y + 15 ) + "px, 0px)";
}
...
Marker.prototype.PointOffScreen = function( target ) {
	...
	//this.label.style.transform = "translate3d(" + ( pos.x + 40 ) + "px, " + ( pos.y + 15 ) + "px, 0px)";
}
...
Marker.prototype.Remove = function() {
	...
	//this.label.AddClass('MarkerFadeOut');
